...
The RIKZ dataset is a great way to look at denominator degrees of freedom. In this experiment, researchers looked at species richness (Richness) across the altitude (NAP) and sun exposure (Exposure). Exposure is the same with each beach. Because you have repeated messurments at the beach level, non-independence at this level should be accounted for, let's use a random effect. In this case, observations at this level (Exposure) should have denominator degrees of freedom that less than the beach level (i.e., less than 9).
...
```{r singularity}
#randomly shuffeling beach around so that it accounts for no variance
RIKZ_Long <-
  RIKZ_Long %>% 
