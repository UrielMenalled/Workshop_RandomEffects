...
As nice it would be to account for Site and Year through a random effect, we don't have enough levels of either group. Remember we need at least three groups per variable for it to be a random effect. Thus, these variables will be fixed effects. On the other hand, block has 16 levels across site-years, it can be a random effect! As for CoverCrop, this is the level of observation, it does not need to be a random effect.
